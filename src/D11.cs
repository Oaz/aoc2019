namespace src11
{
  ...
  {
    public Robot(BigInteger[] program) : base(program) {}

    public Coords CurrentPosition = Coords.At(0,0);
    public int CurrentDirectionIndex = 0;
    static Coords[] Directions = new Coords[] {
      Coords.At(0,-1), Coords.At(-1,0), Coords.At(0,1), Coords.At(1,0)
    };
    public Dictionary<Coords,int> Panel = new Dictionary<Coords, int>();

    public int NumberOfPaintedPanelOnce()
    {
      Paint();
      return Panel.Count();
    }
    public void PaintIdentifier(string fileName)
    {
      Panel[Coords.At(0,0)] = 1;
      Paint();
      var min = Panel.Keys.Min();
      var max = Panel.Keys.Max();
      var size = max-min+Coords.At(1,1);
      var whiteDots = Panel.Where(d => d.Value == 1).Select(d => d.Key-min);
      using (var bitmap = new Bitmap(size.X,size.Y,PixelFormat.Format32bppRgb))
      {
        foreach (var dot in whiteDots)
          bitmap.SetPixel(dot.X,dot.Y,Color.White);
    ...
    {
      if (Panel.TryGetValue(CurrentPosition, out int currentColor))
        return currentColor;
      Panel[CurrentPosition] = 0;
      return 0;
    }
    ...
    public override void WriteToOutput(BigInteger val)
    {
      ...
      if(shouldPaint)
      {
        Panel[CurrentPosition] = (int)val;
        return;
      }
      CurrentDirectionIndex = (CurrentDirectionIndex + 4 + ((val == 0) ? 1 : -1)) % 4;
      CurrentPosition = CurrentPosition+Directions[CurrentDirectionIndex];
    }
  }
